...
static int ov965x_read(struct i2c_client *client, u8 addr, u8 *val)
{
	...
	/* v4l2_err(client, "i2c_read: 0x%02X : 0x%02x. (%d)\n", addr, *val, ret); */
	return ret == 1 ? 0 : ret;
}
...
static int ov965x_write(struct i2c_client *client, u8 addr, u8 val)
{
	...
	/* v4l2_err(client, "i2c_write: 0x%02X : 0x%02x. (%d)\n", addr, val, ret); */
	return ret == 2 ? 0 : ret;
}
			      ...
			      unsigned int len)
{
	...
	for (i = 0; i < len; i++) {
		int ret = ov965x_write(client, regs[i].addr, regs[i].value);
		if (ret < 0)
			return ret;
	}
	return 0;
...
static int __ov965x_set_params(struct ov965x *ov965x)
{
	...
	ret = ov965x_write_array(client, ov965x_init_regs,
				 ARRAY_SIZE(ov965x_init_regs));
	if (ret < 0)
		return ret;
	...
	return ov965x_set_color_matrix(ov965x);
}
		...
		if (on) {
			...
			if (!ret && ov965x->apply_fmt) {
				ret = ov965x_write(client, REG_COM7,
						   ov965x->reg_com7);
				v4l2_dbg(2, debug, client,
					 "%s: on: setting COM7 to: %#x\n",
					 __func__, ov965x->reg_com7);
...
static int ov965x_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_BRIGHTNESS:
		/* err = ov965x_write(client, REG_USER_BRIGHTNESS, ctrl->val); */
		break;

	case V4L2_CID_CONTRAST:
		/* err = ov965x_write(client, REG_USER_CONTRAST, ctrl->val); */
		break;
	...
	case V4L2_CID_POWER_LINE_FREQUENCY:
		err = -EINVAL; //s5k6aa_set_anti_banding(ov965x, ctrl->val);
		break;

	case V4L2_CID_SATURATION:
		/* err = ov965x_write(client, REG_USER_SATURATION, ctrl->val); */
		break;
	}
unlock:
	mutex_unlock(&ov965x->lock);
...
static int ov965x_initialize_ctrls(struct ov965x *ov965x)
{
	...
	ctrls->auto_wb = v4l2_ctrl_new_std(hdl, ops,
				...
				0, 1, 1, 1);
	/* ctrls->gain_red = v4l2_ctrl_new_custom(hdl, &ov965x_ctrls[0], NULL); */
	/* ctrls->gain_green = v4l2_ctrl_new_custom(hdl, &ov965x_ctrls[1], NULL); */
	/* ctrls->gain_blue = v4l2_ctrl_new_custom(hdl, &ov965x_ctrls[2], NULL); */
	/* v4l2_ctrl_auto_cluster(4, &ctrls->awb, 0, false); */

	ctrls->auto_exp = v4l2_ctrl_new_std_menu(hdl, ops,
				V4L2_CID_EXPOSURE_AUTO,
				V4L2_EXPOSURE_MANUAL, 0, V4L2_EXPOSURE_AUTO);
	/* FIXME: Exposure time: x 1 us */
	ctrls->exposure = v4l2_ctrl_new_std(hdl, ops, V4L2_CID_EXPOSURE,
					    0, 6000000U, 1, 100000U);
	/* FIXME: Total gain: 256 <=> 1x */
	/* ctrls->gain = v4l2_ctrl_new_std(hdl, ops, V4L2_CID_GAIN, */
	/* 				0, 256, 1, 256); */
	v4l2_ctrl_auto_cluster(2, &ctrls->auto_exp, 0, false);
