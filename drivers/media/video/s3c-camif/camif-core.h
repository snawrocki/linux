...
#define FMT_FL_S3C64XX_CODEC	(1 << 2)
#define FMT_FL_S3C64XX_PREVIEW	(1 << 3)
 ...
 * @irq:	    interrupt number for the selected data path
 ...
 * @reqbufs_count:  number of the queue buffers requested
 * @scaler:	    the scaler structure
 * @out_fmt:	    pixel format at this video path output
 * @payload:	    an output data frame payload size
 * @out_frame:	    an output pixel resolution
 * @state:	    the video path state
